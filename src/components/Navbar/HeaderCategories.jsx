...
const HeaderCategories = () => {
  ...
  return (
    <div className="text-xs overflow-x-auto flex justify-evenly items-center lg:text-sm">
      {newCategory.map((category,index) => (<span className="capitalize p-2 lg:p-3 hover:bg-purple-200 cursor-pointer" onClick={() => setCategory(category)} onKeyDown={()=>setCategory(category)} key={index}>{category}</span>))}
    </div>
  );
};
